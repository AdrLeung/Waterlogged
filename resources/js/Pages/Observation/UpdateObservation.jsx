...
export default function UpdateObservation({
    ...
    const [form, setForm] = useState({
        ...
        meanLatitude: obs.meanLatitude ?? "",
        meanLongitude: obs.meanLongitude ?? "",
        quantity: obs.quantity ?? "",
        notes: obs.notes ?? "",
        scientificName: obs.scientificName ?? "",
        media: media?.length
            ? media.map((m) => ({ url: m.URL, type: m.mediaType }))
            : [{ url: "", type: "" }],
        projectIds: obs.projectIds ?? [],
    });
    ...
    const handleSubmit = (e) => {
        ...
        router.post(
            route("observation.update", { id: obs.observationID }),
            form,
            {
                onSuccess: () =>
                    addToast("Observation succesfully updated", "success"),
            }
        );
    };
                    ...
                    <div className="gap-4 p-8 bg-white border rounded-lg ">
                        ...
                        >
                            <div>
                                ...
                                <Select
                                    name="location"
                                    value={`${form.meanLongitude},${form.meanLatitude}`}
                                    onValueChange={(value) => {
